...
from lib.utils import torch_max_func, searchsorted, init_weights, calc_weights
...
class NSDE(SampleModel):
    ...
    def __init__(self, sample_model, pre_train=False):
        ...
        self.phi.apply(init_weights)
        # self.nsde_mu.apply(init_weights)
        if pre_train:
            self.pre_train(n_epoch=1000)
    ...
    def nsde_kappa(self, v):
        rho_bar = np.sqrt(1 - self.rho ** 2)
        return rho_bar * self.s(v) * self.phi(v) + self.m(v)
    ...
    def get_girsanov_matrix_diff(self, maturity_idx, vol_path, dW, milstein=True):
        ...
        phi_eval = self.phi(vol_path)
        delta_Z = 1 - phi_eval * dW[:, :, 1, None]
        ...
        if milstein:
            milstein_term = .5 * torch.square(phi_eval) * (torch.square(dW[:, :, 1, None]) - self.dt)
            delta_Z += milstein_term
            delta_Z[:, 0, 0] = 1
        return torch.cumprod(delta_Z, dim=1)[:, maturity_idx].squeeze()

    # def get_girsanov_matrix_diff(self, maturity_idx, vol_path, dW):
    #     vol_path = vol_path.reshape(vol_path.shape[0], vol_path.shape[1], -1)
    #     phi_eval = self.phi(vol_path)
    #     delta_Z = - phi_eval * dW[:, :, 1, None] - .5 * torch.square(phi_eval)
    #
    #     # enforce logZ_0 = 0
    #     delta_Z = torch.cat([torch.zeros((delta_Z.shape[0], 1, delta_Z.shape[2]), device=device), delta_Z],
    #                         dim=1)[:, :-1, :]
    #     return torch.exp(torch.cumsum(delta_Z, dim=1))[:, maturity_idx].squeeze()
    ...
    def pre_train(self, n_epoch=100):
        ...
        with tqdm(range(n_epoch), unit='epoch') as t_epoch:
            for i in t_epoch:
                ...
                self.phi.zero_grad()
                loss = loss_func(self.phi(x), ones*0.1)  # does not learn if pre-trained to "no measure change"
                loss.backward()
                # nn.utils.clip_grad_norm_(self.nsde_params, 1.)
                optimizer_nsde.step()

                t_epoch.set_postfix(loss=loss.item())
                if early_stopping.early_stop or (loss.item() < 5e-5):
                    break
    ...
    def train(self, market_prices, strikes, time_grid, maturity_idx, price_path, vol_path, dW,
        ...
        with tqdm(range(n_epoch), unit='ep') as t_epoch:
            for i in t_epoch:
                ...
                # weights = calc_weights(self.S0, strikes, time_grid[maturity_idx], market_prices, model_prices,
                #                        weights, i=i)
...
if __name__ == '__main__':
    ...
    model = SampleModel(V0=0.02, kappa=3., theta=0.04, nu=.75, rho=-0.7)
    ...
    time_grid = np.linspace(0, maturities[-1], n_steps + 1).astype(np.float32)
    ...
    V = torch.clamp(V, min=5e-6)  # 1e-4 5e-5
    ...
    # before training
    t_time_grid = torch.from_numpy(time_grid).to(device)
    nn_kappa = nsde.nsde_kappa(t_time_grid[:, None]).detach().cpu().numpy()
    ...
    ax[0].plot(time_grid, nn_kappa, '--', label=r'$\kappa^\mathcal{NN}_0$')
    ax[0].plot(time_grid, market_params['kappa'] * (market_params['theta'] - time_grid), 'r-', label='true')
    ...
    nn_kappa = nsde.nsde_kappa(t_time_grid[:, None]).detach().cpu().numpy()
    ax[0].plot(time_grid, nn_kappa, ':', label=r'$\kappa^\mathcal{NN}_n$')
    ax[0].minorticks_on()
